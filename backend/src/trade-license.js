...
export function calculateTradeLicense(input = {}) {
  ...
  if (maxAccountRiskPct == null || maxAccountRiskPct <= 0) {
    errors.push(error("INVALID_ACCOUNT_RISK", "单笔最大账户风险必须大于 0。", ["maxAccountRiskPct"]));
  }
...
export function resolveTradeRecordLifecycle(record, today) {
  if (
    record?.status === "planned"
    && record?.licenseSnapshot
    && /^\d{4}-\d{2}-\d{2}$/.test(String(record?.validForTradeDate ?? ""))
    && String(record.validForTradeDate) < String(today)
  ) {
