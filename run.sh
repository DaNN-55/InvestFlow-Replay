...
cleanup_started_processes() {
  ...
  rm -f "${RUNTIME_DIR}/engine.pid" "${RUNTIME_DIR}/backend.pid" "${RUNTIME_DIR}/web.pid"
}
...
blocked=0
for entry in "Engine:${ENGINE_PORT}" "Backend:${BACKEND_PORT}" "Web:${WEB_PORT}"; do
  service="${entry%%:*}"
  port="${entry##*:}"
...
PYTHONPATH="${APP_DIR}/engine" "${PYTHON_BIN}" -m uvicorn replay_engine.app:app \
  --host 127.0.0.1 --port "${ENGINE_PORT}" >"${RUNTIME_DIR}/engine.log" 2>&1 &
engine_pid=$!
...
wait_for_service "Engine" "${engine_pid}" "http://127.0.0.1:${ENGINE_PORT}/internal/health" "${RUNTIME_DIR}/engine.log"
wait_for_service "Backend" "${backend_pid}" "http://127.0.0.1:${BACKEND_PORT}/api/quant/replay/cache/status" "${RUNTIME_DIR}/backend.log"
wait_for_service "Web" "${web_pid}" "http://127.0.0.1:${WEB_PORT}/decision/market-replay" "${RUNTIME_DIR}/web.log"
trap - INT TERM
